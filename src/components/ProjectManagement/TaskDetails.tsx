...
export default function TaskDetails({ task, updateTask, onClose }: TaskDetailsProps) {
  ...
  const renderViewMode = () => (
    <Box>
      ...
      <Text weight={500} mb="xs">Assigné à:</Text>
      {task.assignedTo ? (
        <Group mb="md">
          <Group spacing="xs">
            <Avatar color="blue" radius="xl">U</Avatar>
            <Text>Utilisateur #{task.assignedTo.id}</Text>
          </Group>
        </Group>
      ) : (
  ...
  const renderEditMode = () => (
    <Box>
      ...
      <Grid mb="md">
        <Grid.Col span={6}>
          <Box>
            <Text size="sm" weight={500} mb={5}>Date d'échéance</Text>
            <DatePicker
              value={editedTask.dueDate ? new Date(editedTask.dueDate) : null}
              onChange={(date) => handleChange('dueDate', date ? date.toISOString() : undefined)}
              allowDeselect
            />
          </Box>
        </Grid.Col>
      ...
      <Select
        ...
        data={[
          ...
          { value: '3', label: 'Utilisateur 3' }
        ]}
        value={editedTask.assignedTo ? editedTask.assignedTo.id.toString() : null}
        onChange={(value) => handleChange('assignedTo', value ? { id: parseInt(value) } : undefined)}
        mb="md"
        searchable
        clearable
      />
